...
class Client:
    def __init__(self):
        self.host = '127.0.0.1'
        self.tam_msg, self.port = server_config()
        self.sock = self.connect_to_server()
        self.estadoDoJogo = EstadoDoJogo.Sem_jogo
    ...
    def renderTable(self):
        ...
        if self.estadoDoJogo != EstadoDoJogo.Sem_jogo:
            table.add_row(["3", "Checar palavra"])
            table.add_row(["4", "Listar palavras digitadas nessa rodada"])
            table.add_row(["5", "Reiniciar jogo atual"])
    ...
    def proccessUserCommand(self, comando_usuario:str)->tuple:
        if comando_usuario == '1':
            comando = "start_game"
            parametro = None
        ...
        elif comando_usuario == '3' and self.estadoDoJogo == EstadoDoJogo.Jogo_em_andamento:
            comando = "check_word"
            comando_usuario = input('Digite a palavra: ')
            parametro = comando_usuario.lower()
            
        elif comando_usuario == '4' and self.estadoDoJogo == EstadoDoJogo.Jogo_em_andamento:
            comando = "list_words"
            parametro = None
            
        elif comando_usuario == '5' and self.estadoDoJogo == EstadoDoJogo.Jogo_em_andamento:
            comando = "restart_game"
            parametro = self.nomeUsuario
    ...
    def sendRequisition(self, req_body) -> json:
        json_data = json.dumps(req_body)
        self.sock.sendall(json_data.encode())
        
        response = self.sock.recv(self.tam_msg)
        response_data = json.loads(response)
        return response_data
    
    def checkEndGame(self) -> bool:
        self.estadoDoJogo = EstadoDoJogo.Jogo_finalizado
        ...
        if usr_input == '1':
            ...
            response_data = self.sendRequisition(req_body)
            response_status = response_data["code_status"]
            
            render_response(response_status, player_name=self.nomeUsuario)
            
            self.estadoDoJogo = EstadoDoJogo.Jogo_em_andamento

            return False
        
        print('')
        
        print(f'Obrigado por jogar {self.nomeUsuario}!')
        print('Feito com ❤️  em 🐍')
        return True
    ...
    def run(self):
        ...
        print('Bem vindo ao jogo de palavras Termo!')
        print('='*50)
        self.nomeUsuario = input('Digite seu nome: ')
        ...
        while True:
            try:
                ...
                comando, parametro = self.proccessUserCommand(cmd_usr)
